...
def test_save_spots_distributed_files():
    ...
    numpy_file_add     = Path('.').resolve() / 'tests' /'testfile.npy'
    format             = 'spots_IDzyx'
    dict_data          = {}
...
def test_load_spots_distributed_files():
    ...
    dict_address_path = Path('.').resolve() / 'tests' /'testfile.npy'
    format = 'spots_IDzyx'
    fp = FileProcessor()
    data = fp.load_spots_distributed_files(dict_address_path, format)
    
    assert np.array_equal(data['one'][['Z','Y','X']].to_numpy(), np.ones((5,3),dtype=int))
    assert np.array_equal(data['four'][['Z','Y','X']].to_numpy(), 4*np.ones((5,3),dtype=int))

 

def test_save_masks_distributed_files():
    
    dict_address_path = Path('.').resolve() / 'tests' / 'testfile_masks.npy'
    fp                = FileProcessor()
    dict_data         = {}
